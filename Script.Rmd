...
The `caTools` library is used to randomly generate train and test data. 75 percent of the data will be used as training data while the other 25 precent will be used as test data.
```{r}
set.seed(101) # set the seed for the reason that always the same sequence will be generated
sample = sample.split(data, SplitRatio = .75) # random sample of 75 percent
train = subset(data, sample == TRUE) # subset the 75 percent data
train =
train_y = train$`data$Score` # store the score in a variable
test  = subset(data, sample == FALSE) # subset the 25 percent data
...
```{r}
plot(pls_model_LOO, ncomp = 4, asp = 1, line = TRUE)
```
