...
### Match SID
The SID column from both data sets will be matched, in order to add the score column to the discriptorData. In the variable `data` is the new dataset stored.
```{r}
# match by SID column
...
RMSE measures how spread out the residuals are. The summary above showed two cross validation estimates, the ordinary CV and the adjCV which is a bias-correctd CV estimate. After inspecting this plot it becomes clear that the third component is the best to choose for future functions.
...
```{r}
# use the plot function for prediction, number of component was selected from the above figure, aspect ratio on 1 to produce a plot where distance between points are accurate, line arguement for regression line
